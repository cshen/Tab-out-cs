...
/* ----------------------------------------------------------------
   CALENDAR — iCal viewer floating panel
   ---------------------------------------------------------------- */

// ICS feed URL — set via config.local.js so each user can point to their own calendar.
// If config.local.js doesn't define it, the calendar panel will show a setup hint.
...
function parseICS(icsText) {
  ...
  const blocks = icsText.split(/BEGIN:VEVENT\s*/i);
  for (let i = 1; i < blocks.length; i++) {
    ...
    for (const raw of lines) {
      ...
      // Strip common parameter suffixes: DTSTART;VALUE=DATE → DTSTART
      key = key.split(';')[0];
      ...
      if (key === 'DTSTART' || key === 'DTEND') {
        ev[key.toLowerCase()] = parseICalDate(val);
      } else if (key === 'SUMMARY') {
        ev.title = val;
    ...
    if (ev.dtstart) {
      // If it's an all-day event (no time component), make end inclusive
      const allDay = ev.dtstart.length === 8; // YYYYMMDD format
      if (ev.dtend && allDay) {
        ev.title = ev.title || 'Untitled';
        events.push({
          title: ev.title,
          start: formatICalDate(ev.dtstart),
          end: formatICalDate(ev.dtend),
          allDay: true,
          extendedProps: { location: ev.location || '', description: ev.description || '' },
        });
      } else if (ev.dtstart.length >= 15) {
        // Timed event
        ev.title = ev.title || 'Untitled';
        events.push({
          title: ev.title,
          start: formatICalDateTime(ev.dtstart),
          end: ev.dtend ? formatICalDateTime(ev.dtend) : formatICalDateTime(ev.dtstart),
          extendedProps: { location: ev.location || '', description: ev.description || '' },
        });
      }
    }
...
function parseICalDate(val) {
  // Remove timezone suffix: 20260425T090000Z → strip Z, or 20260425
  return val.replace(/Z$/i, '');
}

function formatICalDate(yyyymmdd) {
  // YYYYMMDD → YYYY-MM-DD
  return yyyymmdd.substring(0, 4) + '-' + yyyymmdd.substring(4, 6) + '-' + yyyymmdd.substring(6, 8);
}

function formatICalDateTime(yyyymmddThhmmss) {
  // YYYYMMDDTHHMMSS → YYYY-MM-DDTHH:MM:SS
  const d = yyyymmddThhmmss.substring(0, 8);
  const t = yyyymmddThhmmss.length >= 15 ? yyyymmddThhmmss.substring(9, 15) : '000000';
  return formatICalDate(d) + 'T' + t.substring(0, 2) + ':' + t.substring(2, 4) + ':' + t.substring(4, 6);
}
...
function initCalendar() {
  ...
  if (typeof FullCalendar === 'undefined') {
    calendarEl.innerHTML = '<div style="padding:24px;text-align:center;color:var(--muted);font-size:13px">Calendar is loading...</div>';
  ...
  if (!ICS_FEED_URL) {
    calendarEl.innerHTML = `<div style="padding:24px;color:var(--muted);font-size:13px;text-align:center;line-height:1.6">
      No calendar URL configured.<br>
      Set <code style="font-size:12px;background:var(--warm-gray);padding:1px 6px;border-radius:3px">LOCAL_ICS_FEED_URL</code> in <code style="font-size:12px;background:var(--warm-gray);padding:1px 6px;border-radius:3px">config.local.js</code>.
    </div>`;
    return;
  }
...
// Re-theme calendar when theme changes (update button colors)
const calendarThemeObserver = new MutationObserver(() => {
  if (!calendarInstance) return;
  // Re-render toolbar buttons to pick up new CSS variable values
  calendarInstance.render();
});
